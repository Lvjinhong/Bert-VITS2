...
lines=sorted(lines,key=lambda x:int(x.split('|')[0].split('/')[-1].split('_')[-1][:-4]))
...
for i, line in enumerate(tqdm(lines)):
    ...
    #写入dataset list文件
    f_list_res.append('|'.join([os.path.join(base_path2,new_name)]+line.split('|')[1:]))
    #重命名文件
...
with open(list_path,'w+',encoding='utf-8') as f1 ,open(final_list_path,'w+',encoding='utf-8') as f2:
